...
export class SheetsMigration {
    sheets;
    db;
    constructor(sheetsConfig, databaseConfig) {
    ...
    async migrateTrainingData() {
        ...
        try {
            ...
            for (const training of trainingData) {
                if (!training.phoneNumber || !training.timestamp)
                    continue;
                try {
                    // Get user ID
    ...
    async migrateConversations() {
        ...
        try {
            ...
            for (const conversation of conversations) {
                if (!conversation.phoneNumber || !conversation.timestamp)
                    continue;
                try {
                    // Get user ID
                    const user = await this.db.query.select()
    ...
    parseTimeToSeconds(timeStr, unit) {
